const App__appHeader = {
  ...
  init: function () {
    ...
    this.smallScreenNavPanel          = document.querySelector('[data-hook=appHeader__smallScreenNavPanel]');
		this.smallScreenNavPanelDrawer    = document.querySelector('[data-hook=appHeader__smallScreenNavPanelDrawer]');
		this.smallScreenNavPanelTrigger		= document.querySelector('[data-hook=appHeader__smallScreenNavPanelTrigger]');
    
    if ( this.smallScreenNavPanel && this.smallScreenNavPanelTrigger && this.smallScreenNavPanelDrawer ) {
			
			this.smallScreenNavPanelTriggerClose = this.smallScreenNavPanelTrigger.querySelector('[data-hook=appHeader__smallScreenNavPanelTriggerClose]')
			this.smallScreenNavPanelTriggerOpen = this.smallScreenNavPanelTrigger.querySelector('[data-hook=appHeader__smallScreenNavPanelTriggerOpen]')
      ...
      document.addEventListener('click', (e) => {
        if ( !e.target.matches('[data-hook=appHeader__wantTo]') ) {
					...
					if ( this.element && this.element.querySelector( "[data-hook=appHeader__wantToWrapper].is-open" ) ) {
						this.wantTo__close();
					}
        ...
        let appHeader__wantToWrapper = this.element.querySelector('[data-hook=appHeader__wantToWrapper]');
      ...
      document.addEventListener('click', (e) => {
        ...
        let appHeader__globalSearchPanel = this.element.querySelector('[data-hook=appHeader__globalSearchPanel]');
      ...
      this.element.addEventListener('click', (e) => {
        ...
        if ( !e.target.matches('[data-hook=appHeader__primaryNavItemPanelTrigger]') ) {
					if (this.element && this.element.querySelector("[data-hook=appHeader__primaryNavItem].is-open")) {
      			this.primaryNavItemPanel__close(this.element.querySelector("[data-hook=appHeader__primaryNavItem].is-open"));
    			}
          return false;
        }
        e.preventDefault();
  ...
  primaryNavItemPanel__open: function ( primaryNavItem ) {
    ...
    let openPanel = this.element.querySelector('[data-hook=appHeader__primaryNavItem].is-open');
    if ( openPanel ) {
  ...
  globalSearchPanel__open: function () {
    // console.log('this.globalSearchPanel__open()');
    ...
    this.element.querySelector('[data-hook=appHeader__smallScreenNavPanelHeader]').classList.add('is-back-to-main');
    ...
    this.element.querySelectorAll(selectors.join()).forEach((item) => item.setAttribute('tabindex', '-1'));
		...
		this.element.querySelectorAll(selectors.join()).forEach((item) => item.removeAttribute('tabindex'));
    this.focusTrap = App.utils.focusTrap.createFocusTrap('[data-hook=appHeader]', {
        initialFocus: this.element.querySelector('[data-hook=globalSearch__input]'),
        allowOutsideClick: true,
      }
    );
  ...
  globalSearchPanel__close: function () {
    // console.log('this.globalSearchPanel__close()');

    let globalSearchPanel = document.querySelector('[data-hook=appHeader] [data-hook=appHeader__globalSearchPanel]');
    globalSearchPanel.setAttribute('aria-hidden', true);

    this.element.querySelector('[data-hook=appHeader__smallScreenNavPanelSearchButton]').style.display = "inline";
    this.element.querySelector('[data-hook=appHeader__smallScreenNavPanelBackButton]').style.display = "none";

    this.element.querySelector('[data-hook=appHeader__smallScreenNavPanelHeader]').classList.remove('is-back-to-main');
    ...
    if ( App.mediaQueries.isLargeUp ) {
      ...
      if ( this.focusTrap ) {
        this.focusTrap.deactivate({
          onDeactivate: this.element.querySelector('[data-hook=appHeader__globalSearchPanelTrigger]').focus(),
        });
      }
    } else {
      if (this.focusTrap) {
        this.focusTrap.deactivate({
          onDeactivate: this.element.querySelector('[data-hook=appHeader__smallScreenNavPanelSearchButton]').focus(),
        });
      }
    }
  },
  ...
  globalSearch__submit: function () {
    // console.log('this.globalSearch__submit()');
    let globalSearchPanel = document.querySelector('[data-hook=appHeader] [data-hook=appHeader__globalSearchPanel]');
